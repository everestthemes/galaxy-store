 ...
 * @return void
 */
function galaxy_store_woocommerce_setup() {
	add_theme_support(
		'woocommerce',
		array(
...
add_action( 'after_setup_theme', 'galaxy_store_woocommerce_setup' );
...
function galaxy_store_woocommerce_scripts() {
	...
	wp_add_inline_style( 'galaxy-store-woocommerce-style', $inline_font );
}
add_action( 'wp_enqueue_scripts', 'galaxy_store_woocommerce_scripts' );
 ...
 */
function galaxy_store_woocommerce_active_body_class( $classes ) {
	$classes[] = 'woocommerce-active';

	return $classes;
}
add_filter( 'body_class', 'galaxy_store_woocommerce_active_body_class' );
 ...
 */
function galaxy_store_woocommerce_related_products_args( $args ) {
	$defaults = array(
		'posts_per_page' => 3,
		'columns'        => 3,
	);
...
add_filter( 'woocommerce_output_related_products_args', 'galaxy_store_woocommerce_related_products_args' );
...
if ( ! function_exists( 'galaxy_store_woocommerce_wrapper_before' ) ) {
	/**
	 * Before Content.
	 *
...
add_action( 'woocommerce_before_main_content', 'galaxy_store_woocommerce_wrapper_before' );

if ( ! function_exists( 'galaxy_store_woocommerce_wrapper_after' ) ) {
	/**
	 * After Content.
	 *
...
add_action( 'woocommerce_after_main_content', 'galaxy_store_woocommerce_wrapper_after' );
 ...
 *
	<?php
		if ( function_exists( 'galaxy_store_woocommerce_header_cart' ) ) {
			galaxy_store_woocommerce_header_cart();
		}
	?>
 */

if ( ! function_exists( 'galaxy_store_woocommerce_cart_link_fragment' ) ) {
	/**
	 * Cart Fragments.
	 *
	 ...
	 */
	function galaxy_store_woocommerce_cart_link_fragment( $fragments ) {
		ob_start();
